...
Adding the command switch will add a CNAME entry in the CloudDNS pointing FQDN at the created database instance. If there is a CNAME record already for that FQDN, it is updated to point to the newly created instance.

  [ -l | --ttl SECONDS ]

Sets the Time To Live of the CNAME record
